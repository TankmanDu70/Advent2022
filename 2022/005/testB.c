...
void printDock(dock *dock)
{
    ...
    for (size_t pos = 0; pos <= highest_stack; pos++)
    {
...
void permutate(dock *dock, uint8_t count, uint8_t origin, uint8_t destination)
{
    dock->docks[destination].top += count;
    for (uint8_t i = 0; i < count; i++)
    {
        ...
        dock->docks[destination].list[dock->docks[destination].top - count + i] = dock->docks[origin].list[dock->docks[origin].top - count + i];
        //printf("%c", dock->docks[origin].list[dock->docks[origin].top - count + i]);
        dock->docks[origin].list[dock->docks[origin].top - count + i] = 0;
