        ...
        for (size_t dir = 0; dir < 4; dir++)
        {
            if (nod->can[nod->pref[dir]] && (nod->options > 0))
                if ((nod->tried[nod->pref[dir]] == false))
                    ...
                    {
                        path[nod->y][nod->x] = charDir[nod->pref[dir]];
                        nod->opt_next = getNodeByXY(nod, nod->x + dirMod[nod->pref[dir]].x, nod->y + dirMod[nod->pref[dir]].y);
                        nod->opt_next->id = nod->id + 1;
                        nod->options--;
                        nod->tried[nod->pref[dir]] = true;
                        cX = nod->x;
                        cY = nod->y;
...
bool beenThere(struct node *nod, char dir)
{
    return (isInList(nod->x + dirMod[dir].x, nod->y + dirMod[dir].y)); //| (path[nod->y + dirMod[dir].y][nod->x + dirMod[dir].x] != '.');
}
...
void addNode(struct node *nod, uint _x, uint _y, char from)
{
    ...
    node_cnt++;
    nod->opt_next = nod->next;
    nod->next->prev = nod;
    nod->next->opt_prev = nod;
    nod->next->x = nod->x + _x;
    nod->next->y = nod->y + _y;
    nod->next->next = NULL;
    nod->next->opt_next = NULL;
...
void main(int argc, char *argv[])
{
    ...
    while (Cnode->height < 27)
    {
        ...
        system("clear");
        if (findWay(Cnode) == 1)
            Cnode = Cnode->opt_next;
        else if (Cnode->prev != NULL)
            Cnode = Cnode->opt_prev;
        else
            exit(1);
        // printf("After? ");
