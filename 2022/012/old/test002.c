...
struct node
{
    ...
    struct node *next;
    struct node *prev;
    struct node *opt_next;
    struct node *opt_prev;
};
...
void printNode(struct node *nod)
{
    printf("%3dth node %x (%2d:%2d) h=%2d (%2c) --options %d (%d=%c)(%d=%c)(%d=%c)(%d=%c) -tried(l%d)(t%d)(r%d)(d%d)\n",
           nod->id, nod, nod->x + 1, nod->y + 1, nod->height, nod->height + 'a' - 1, nod->options, nod->can[left], nod->prox[left] + 'a' - 1, nod->can[top], nod->prox[top] + 'a' - 1, nod->can[right], nod->prox[right] + 'a' - 1, nod->can[down], nod->prox[down] + 'a' - 1, nod->tried[left], nod->tried[top], nod->tried[right], nod->tried[down]);
}
...
int findWay(struct node *nod)
{
    ...
    if (nod->options > 1) //! time for a decision of where we go...
    {
        ...
        {
            if (nod->can[nod->pref[dir]])
                if ((nod->tried[nod->pref[dir]] == false) && canGo(nod, nod->pref[dir]) && (!beenThere(nod, nod->pref[dir])))
                {
...
bool beenThere(struct node *nod, char dir)
{
    return isInList(nod->x + dirMod[dir].x, nod->y + dirMod[dir].y);
}
...
void checkOptions(struct node *nod)
{
    ...
    if (nod->y + 1 < mapH) //! down
    {
        ...
        if (((nod->prox[down] - nod->height) <= 1))
        {
            nod->options += 1;
            nod->can[down] = true;
            struct node *n = getNodeByXY(nod, nod->x + dirMod[down].x, nod->y + dirMod[down].y);
            if (n != NULL)
                if ((n->id < nod->id) && (n != nod->prev) && (nod->height - n->height <= 1))
                {
                    printf("\033[0;31m");
                    printf("SHORTCUT!!!");
                    n->next = nod;
    ...
    if (nod->y > 0) //! top
    {
        ...
        if (((nod->prox[top] - nod->height) <= 1))
        {
            nod->options += 1;
            nod->can[top] = true;
            struct node *n = getNodeByXY(nod, nod->x + dirMod[top].x, nod->y + dirMod[top].y);
            if (n != NULL)
                if ((n->id < nod->id) && (n != nod->prev) && (nod->height - n->height <= 1))
                {
                    printf("\033[0;31m");
    ...
    if (nod->x > 0) //! left
    {
        ...
        if (((nod->prox[left] - nod->height) <= 1))
        {
            nod->can[left] = true;
            nod->options += 1;
            struct node *n = getNodeByXY(nod, nod->x + dirMod[left].x, nod->y + dirMod[left].y);
            if (n != NULL)
                if ((n->id < nod->id) && (n != nod->prev) && (nod->height - n->height <= 1))
                {
                    printf("\033[0;31m");
    ...
    if (nod->x + 1 < mapL) //! right
    {
        ...
        if (((nod->prox[right] - nod->height) <= 1))
        {
            nod->can[right] = true;
            nod->options += 1;
            struct node *n = getNodeByXY(nod, nod->x + dirMod[right].x, nod->y + dirMod[right].y);
            if (n != NULL)
                if ((n->id < nod->id) && (n != nod->prev) && (nod->height - n->height <= 1))
                {
                    printf("\033[0;31m");
